...
class YoutubeErrorHandler(StateErrorHandler):
    async def handle(self):
        ...
        if isinstance(self.event.exception, DownloadError):
            ...
            if "Unable to download API page: HTTP Error 404" \
                ...
                return await self.bot(
                    SendMessage(
                        chat_id=self.event.update.message.chat.id,
                        text=_("Видео недоступно для скачивания. Ошибка 404")
                    )
                )
            ...
            if "Telegram server says ClientOSError: [Errno 2]" \
               " Can not write request body " in self.event.exception.message:
                return await self.bot(
                    SendMessage(
                        chat_id=self.event.update.message.chat.id,
                        text=_("Ошибка из-за того, что это видео сейчас "
                               "кто-то скачивал. Пожалуйста, повторите попытку "
                               "через 5 минут.")
                    )
                )
        ...
        return await self.bot(
            SendMessage(
                chat_id=self.event.update.message.chat.id,
                text=_("Произошла какая-то ошибка (никто не знает, из-за чего "
                       "она произошла). Сообщите админам")
            )
        )
