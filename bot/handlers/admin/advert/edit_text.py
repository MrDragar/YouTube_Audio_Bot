...
class GetIdHandler(StateMassageHandler):
    async def handle(self):
        ...
        await self.bot(
            SendMessage(
                chat_id=self.chat.id, text="Напишите новый текст для рекламы"
            )
        )
...
class GetNewTextHandler(StateMassageHandler):
    async def handle(self):
        ...
        error = await edit_advert_text(
            advert_id=advert_id, chat_id=self.chat.id,
        ...
        return await self.bot(
            SendMessage(
                chat_id=self.chat.id, text="Текст рекламы успешно обновлён"
            )
        )
